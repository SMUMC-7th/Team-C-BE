...
public interface BookmarkPolicyRepository extends JpaRepository<BookMarkPolicy, Long> {
    ...
    List<BookmarkResponseByDateDTO> findByMemberIdAndDateWithinPolicyPeriod(@Param("memberId") Long memberId, @Param("date") LocalDate date);
    ...
    @Query("SELECT new umc.teamc.youthStepUp.calendar.dto.response.BookmarkResponseByMonthDTO(b.id,b.policy.polyBizSjnm, b.policy.startDate, b.policy.deadline) " +
            ...
            "AND b.deletedAt IS NULL")
    List<BookmarkResponseByMonthDTO> findByMemberIdAndMonthWithinPolicyPeriod(@Param("memberId") Long memberId, @Param("month") LocalDate month);
    ...
    @Query("SELECT new umc.teamc.youthStepUp.profile.dto.response.BookmarkResponseDTO(b.id, p.srchPolicyId, p.polyBizSjnm) " +
            ...
            "AND b.deletedAt IS NULL " +
            "ORDER BY b.id DESC")
    Slice<BookmarkResponseDTO> findByMemberIdAndDeletedAtIsNullOrderByIdDesc(@Param("memberId") Long memberId, Pageable pageable);
